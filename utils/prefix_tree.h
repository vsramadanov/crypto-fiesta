...
class prefix_tree {
    ...
    void match(const std::string &target, Node *root, size_t k,
               std::string &key, std::vector<std::string> &storage) {
        if (root == nullptr)
            return;
     ...
     */
    template <typename Func> void visit(Node *root, Func &&f) {
        if (root == nullptr)
            return;
        for (auto *ptr : root->next)
            visit(ptr, f);
  ...
  public:
    ...
    bool contains(const std::string &key) { return contains(&root, key, 0); }
                               ...
                               std::string>               // reference
    {
        ...
        void dfs() {
            while (!st.empty()) {
                ...
                for (auto idx : std::views::iota(0, R) | std::views::reverse) {
                    ...
                    if (w)
                        st.push(
                            {w, static_cast<char>('a' + idx), descr.level + 1});
                }
      ...
      public:
        explicit const_iterator(const Node *node, bool end = false)
            : root(node) {
            if (end)
                return;
